...
// Use Google DNS [Domain Name System] to resolve MongoDB SRV connection issues
...
const categoriesData = [
  { name: 'beauty', description: 'Premium cosmetics, makeup, and skincare products' },
  { name: 'fragrances', description: 'Luxurious perfumes and Eau De Toilette fragrances' },
  { name: 'furniture', description: 'Timeless home, bedroom, and office furniture' },
  { name: 'groceries', description: 'Fresh fruits, meats, pet food, and kitchen essentials' }
];
...
const testimonialsData = [
  {
    ...
    rating: 5,
    comment: "I stumbled upon this online store while searching for home essentials, and I couldn't be happier with my experience! The selection is great and delivery is blazing fast. Highly recommended!"
  },
  {
    name: 'Thomas John',
    rating: 5,
    comment: "This store is my go-to for all my daily groceries and beauty care! They always deliver fresh products and customer support is exceptional. Trust me, you won't regret shopping here!"
  },
  {
    name: 'Kevin Bryan',
    rating: 5,
    comment: "I recently purchased a bedside table and some fragrance sets from here, and I'm absolutely thrilled with my purchase! Exceptional quality and premium packaging."
  }
];
...
const runSeeder = async () => {
  try {
    ...
    const admin2 = await User.create({
      ...
      role: 'Admin',
      phone: '1234567895',
      address: '999 Admin Blvd, Boston MA'
    });

    const seller = await User.create({
      name: 'Seller User',
      email: 'seller@smartstore.com',
      password: hashedPassword,
      role: 'Seller',
      phone: '1234567891',
    ...
    const seller2 = await User.create({
      ...
      role: 'Seller',
      phone: '1234567896',
      address: '888 Seller Ave, Portland OR'
    });
    ...
    const customer2 = await User.create({
      ...
      email: 'customer2@smartstore.com',
      password: hashedPassword,
      role: 'Customer',
      phone: '1234567897',
      address: '111 Customer Way, Miami FL'
    });
    ...
    for (const cat of categoriesData) {
      const created = await Category.create({
        ...
        image: `https://picsum.photos/seed/${cat.name}/500/500`
      });
      categoriesMap[cat.name] = created._id;
    }

    // 5. Seed Products
    console.log('Seeding 30 DummyJSON products...');
    ...
    for (const p of rawProducts) {
      ...
      const created = await Product.create({
        ...
        reviews: p.reviews || [],
        meta: p.meta || {},
        sales: Math.floor(Math.random() * 50) + 5, // Random sales number
    ...
    const ordersData = [
      {
        user: customer._id,
        items: [
          { product: seededProducts[5]._id, quantity: 1, price: seededProducts[5].price }, // CK One
          { product: seededProducts[1]._id, quantity: 1, price: seededProducts[1].price }  // Eyeshadow Palette
        ],
        totalAmount: seededProducts[5].price + seededProducts[1].price,
      ...
      },
      {
        user: customer._id,
        items: [
          { product: seededProducts[0]._id, quantity: 2, price: seededProducts[0].price } // Lash Princess Mascara
        ],
        totalAmount: seededProducts[0].price * 2,
        shippingAddress: 'Customer User, 789 Customer Road, Orlando FL, Phone: 1234567892',
        paymentMethod: 'cod',
      ...
      },
      {
        user: customer._id,
        items: [
          { product: seededProducts[15]._id, quantity: 3, price: seededProducts[15].price } // Apple
        ],
        totalAmount: seededProducts[15].price * 3,
        shippingAddress: 'Customer User, 789 Customer Road, Orlando FL, Phone: 1234567892',
        paymentMethod: 'card',
        paymentStatus: 'paid',
