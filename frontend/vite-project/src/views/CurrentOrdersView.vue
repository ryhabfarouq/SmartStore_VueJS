...
const fetchOrders = async () => {
  ...
  try {
    ...
    // Filter active/current orders
    orders.value = data.filter((order: any) => 
      !['Delivered', 'Cancelled'].includes(order.status)
    )
  } catch (err: any) {
    console.error('Error fetching orders:', err)
    errorMsg.value = err.message || 'Something went wrong while loading orders.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 font-sans">
    ...
    <div class="flex items-center text-xs text-gray-500 gap-1.5 uppercase font-medium tracking-wide mb-8">
      ...
      <span>/</span>
      <span class="text-gray-900 font-semibold">Current Orders</span>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 bg-white border border-gray-100 p-6 sm:p-8 rounded-lg shadow-sm text-left">
        ...
        <div v-if="loading" class="py-12 flex flex-col items-center justify-center text-gray-400 gap-3">
          ...
          <span class="text-sm font-medium">Retrieving active orders from database...</span>
        </div>
        ...
        <div v-else-if="orders.length === 0" class="py-16 text-center">
          ...
          <p class="text-sm text-gray-500 mt-1 max-w-sm mx-auto">
            You don't have any ongoing purchases right now. You can check historical orders under Order History.
          </p>
          <router-link 
            to="/shop" 
            ...
            <div class="px-6 py-6 border-t border-gray-100 bg-white">
              <div class="max-w-xl mx-auto">
                <div class="flex items-center justify-between relative">
                  ...
                  <div 
                    class="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-pink-500 z-0 transition-all duration-500"
                    :style="{ width: `${((getStepNumber(order.status) - 1) / 3) * 100}%` }"
                  ></div>
                  ...
                  <div class="flex flex-col items-center gap-1.5 relative z-10">
                    <div 
                      ...
                      :class="getStepNumber(order.status) >= 1 
                        ? 'bg-pink-500 text-white ring-4 ring-pink-500/10' 
                        : 'bg-white border-2 border-gray-200 text-gray-400'"
                    >
                      1
                    </div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-gray-500">Preparing</span>
                  </div>
                  ...
                  <div class="flex flex-col items-center gap-1.5 relative z-10">
                    <div 
                      class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300"
                      :class="getStepNumber(order.status) >= 2 
                        ? 'bg-pink-500 text-white ring-4 ring-pink-500/10' 
                        : 'bg-white border-2 border-gray-200 text-gray-400'"
                    >
                      2
                    </div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-gray-500">Packaged</span>
                  </div>
                  ...
                  <div class="flex flex-col items-center gap-1.5 relative z-10">
                    <div 
                      class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300"
                      :class="getStepNumber(order.status) >= 3 
                        ? 'bg-pink-500 text-white ring-4 ring-pink-500/10' 
                        : 'bg-white border-2 border-gray-200 text-gray-400'"
                    >
                      3
                    </div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-gray-500">In Delivery</span>
                  </div>
                  ...
                  <div class="flex flex-col items-center gap-1.5 relative z-10">
                    <div 
                      class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300"
                      :class="getStepNumber(order.status) >= 4 
                        ? 'bg-pink-500 text-white ring-4 ring-pink-500/10' 
                        : 'bg-white border-2 border-gray-200 text-gray-400'"
                    >
                      4
                    </div>
                    <span class="text-[10px] font-bold uppercase tracking-wider text-gray-500">Delivered</span>
                  </div>
                </div>
              </div>
            </div>
